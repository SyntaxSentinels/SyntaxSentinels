...
April 4 & 2.0 & Final documentation\\
% Date 2 & 1.1 & Notes\\
\bottomrule
\end{tabularx}
...
\begin{itemize}
  \item \textbf{User Manual for Instructors and Administrators}: 
  The user manual will provide detailed instructions on how to use the plagiarism detection system. It will include:
  \begin{itemize}
    \item Step by step instructions for setting up the system.
    \item Instructions on how to run the servers
    \item Step-by-step instructions for uploading code snippets and generating reports.
    \item Troubleshooting tips for common issues.
  \end{itemize}

  \item \textbf{Benchmarking Against MOSS}: 
  The system’s performance will be benchmarked against MOSS (Measure of Software Similarity) to evaluate its effectiveness. This will involve:
  \begin{itemize}
    \item Running the same set of code snippets through both systems.
    \item Comparing similarity scores and false positive rates.
    \item Documenting the results in a performance report.
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    follow SOLID principles and the 
    existence of documentation and tests.
    \item If there are components that don't, pther team members will be consulted
    to discuss issues with that component.
  \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
  ...
  Input:  Deploy the system on a free hosting service (e.g. Heroku, Netlify). Will be done using AWS for consistency in team's own testing.
...
\begin{enumerate}
  ...
  How test will be performed:
  Release the model and verify that a report is generated with relevant metrics such plagiarism similarity scorings across datasets (seen in bench mark document). 
...
\begin{enumerate}
  ...
  and returns a success HTTP 200 response.
