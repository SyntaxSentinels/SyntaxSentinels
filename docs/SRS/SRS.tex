...
April 4th, 2025 & 1.0 & Updates after Rev1\\
% Date 2 & 1.1 & Notes\\
\bottomrule
\end{tabularx}
...
\begin{table}[h!]
    ...
    \hline
    Our product shall store any user-uploaded data securely and allow users to request its deletion. &
...
\begin{itemize}
  \item The system must ensure compliance with data protection regulations by storing data securely and allowing users to request data deletion.
...
\begin{center}
    \includegraphics[scale=0.6]{assets/UseCase.drawio.png}
\end{center}
...
\begin{itemize}
    ...
    \textbf{Justification}: User's must upload code snippets for the application to analyze.\\
    \textbf{Fit Criterion}: System successfully accepts code snippet uploads.
    ...
    \textbf{Justification}: Detecting plagiarism based on a threshold is the main approach for this project.\\
    \textbf{Fit Criterion}: Plagiarism identification is produced using the threshold.
    ...
    \textbf{Justification}: New users may need help understanding the output of the application .\\
    \textbf{Fit Criterion}: Documentation with relevant information exists.
    \item \textbf{FR-4}: The system shall allow users \( u \in U \) to flag snippets \( s \in S \), creating a flagged set \( P \subseteq S \).\\
    \textbf{Justification}: The user may want to save identified pairs for further investigation.\\
    \textbf{Fit Criterion}: Snippets can be flagged in application.
    \item \textbf{FR-5}: The system shall output a set of similarity scores \( \{ \text{Sim}(s_i, s_j) \mid s_i, s_j \in S \} \).\\
    \textbf{Justification}: Similarity scores between code snippets are the main outputs that the user will see.\\
    \textbf{Fit Criterion}: System successfully outputs similarity scores.
    \item \textbf{FR-6}: The system shall generate reports \( R \) summarizing \( \text{Sim}(s_i, s_j) \) and \( t \in T \), where \( R \neq \emptyset \).\\
    \textbf{Justification}: The report is what the user will main use to interpret the results of the application.\\
    \textbf{Fit Criterion}: Reports are successfully generated.
    ...
    \textbf{Justification}: Users will want to have an account so they can keep the plagiarism reports they have stored.\\
    \textbf{Fit Criterion}: Accounts can be made, and reports become associated with those accounts.
    \item \textbf{FR-8}: The system shall allow \( u \in U \) to authenticate with \( \texttt{email} \) and \( \texttt{password} \).\\
    \textbf{Justification}: E-mail and password are standard ways to create accounts.\\
    \textbf{Fit Criterion}: Accounts can be made using an E-mail and password.
    \item \textbf{FR-9}: The system shall visualize \( z \in Z \) via a web interface.\\
    \textbf{Justification}: There is no need for the application to be it's own software. It is enough to have it on the web.\\
    \textbf{Fit Criterion}: Application's final product is a web page.
\end{itemize}
...
Integrity requirements are outside the scope of this project.

\subsection{Privacy Requirements}
\begin{itemize}
    \item \textbf{SR-P1: Secure Data Storage}: The system shall store user-uploaded data in a secure manner.
...
\begin{itemize}
    ...
    Justification - No sensitive user-generated data should be logged, since the logs are not intended to be opaque or secure. Logging system events and user actions provide a transparent way to track system usage and ensure accountability without violating the secure data storage policy.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Secure Data Storage}: User data will be stored securely and only until it is requested to be deleted by the user.
        All user-uploaded data will be stored in a way that it is only accessible by the user who uploaded it.
        The data will be deleted immediately if requested by the user at any time.
