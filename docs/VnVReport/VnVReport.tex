...
\begin{longtable}[c]{|p{1.5cm}|p{1.5cm}|p{2.5cm}|p{2.5cm}|p{2.5cm}|p{1.5cm}|}
    ...
    alt text and all text meets contrast and sizing requirements using Chrome DevTools. 
    !Specify visibility score here potentiall or other metric! & Pass \\
    \hline
    ...
    have not met any input size that has forced processing time that long. !Be sure we 
    have this implemented!& Pass \\
    \hline
    ...
    less than 5\% false positives. & !state this has been met or specify further testing that
    ideally we want to do with our produced datasets!  & Pass \\
    \hline
    test-PR-6 & PR-SE1 & Developer adds a new feature, and all existing features are tested
     ...
     new feature was added. !Have regression tests online to check functionality when PR is 
     made! & Pass \\
    \hline
    ...
    and display a confirmation message. & Cloud has yet to be connected. It is later stage 
    ...
    althought future releases will have a new report attached & Fail !need action 
    specifying this change with benchmakring! \\
    \hline
