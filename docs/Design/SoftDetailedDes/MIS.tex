...
April 4 & 2.0 & Initial documentation\\
\bottomrule
\end{tabularx}
...
\begin{itemize}
    ...
    \item \textbf{input:} Path representing location of snippets on user computer.
    \item \textbf{output:} Returns \texttt{true} if the file meets \texttt{MAX\_FILE\_LENGTH} and 
...
\section{MIS of NLP Module}  \label{NLPModule} %Module start here ####
...
\section{MIS of Tokenization Module} \label{smTokenization}
...
\section{MIS of Similarity Scoring Module} \label{mScoring}
...
\section{MIS Report Generation Module} \label{mReport}
...
\begin{itemize}
    \item Code Upload Module
    \begin{itemize}
        \item All files dealt with locally to directly address user
    \end{itemize}

    \item Threshold Module
    \begin{itemize}
        \item Errors are propogated to code upload module where threshold decision was made
    \end{itemize}
    ...
    \item  Report Resules Module
    \begin{itemize}
        \item Errors are dealt with locally but error code is passed in API response for frontend to acknowledge
    \end{itemize}

    \item  Abstract Model Module
    \begin{itemize}
        \item Errors are propogated to NLP module to allow it to make further assessment decisions on whether to continue or try again.
    \end{itemize}
...
The following interpretations can substitute anywhere a variable in an exported access program of the same name is found.
\begin{itemize}
    \item snippets or data: JSON
    \begin{itemize}
        \item contains str-str pairings representing file names and their corresponding stringified code
    \end{itemize}
    \newpage
    \item report: JSON
    \begin{itemize}
        \item contains list of pairwise plagiarism scores assigned as dict objects with filename1 and filename2 to identify the pairing and a float from 0 to 1 for the score, as well as plagiarism statuses for each individual file (also a float from 0 to 1) and corresponding color information for UI relating to identification of similarity
    \end{itemize}
