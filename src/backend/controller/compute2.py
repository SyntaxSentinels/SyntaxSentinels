...
def fix_python2_to_python3(code):
    ...
    pattern = r'print(?!\()(\s*)(?!\w)(.*)'# Match print without parentheses immediately after it

    # Replacement will wrap the matched part (arguments) in parentheses
    replacement = r'print(\2)'  # Wrap the arguments (captured by (.*)) in parentheses
...
class ASTSimilarity:
    ...
    def compute(self, code1, code2):
        ...
        # Apply the 2to3 transformation

        return float(self.compute_similarity(code1, code2))
...
class EmbeddingSimilarity:
    ...
    def compute(self, code1, code2):
        ...
        cosine_sim = torch.nn.functional.cosine_similarity(
            code1.unsqueeze(0), code2.unsqueeze(0)
        ).item()
...
def compute_similarities_from_zip(zip_bytes, model_name="microsoft/codebert-base"):
    ...
    def process_pair(pair):
        ...
        return {
            "file1": fname1,
            "file2": fname2,
            "similarity_score": [embed_sim, ast_sim, token_sim]
        }
